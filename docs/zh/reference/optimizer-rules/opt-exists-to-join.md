---
id: zh-optimizer-rule-opt-exists-to-join
...
- zh
- rewrite
description: EXISTS 子查询向外层查询返回一个布尔值，表示是否存在满足条件的行。当满足一定条件时（等值关联、子查询结果唯一、无分组无 LIMIT），EXISTS
  子查询可以转换为 JOIN 表连接。转换为 JOIN 后，数据库优化器可以灵活选择驱动表，生成更优的查询计划，从而提升查询性能。
---
