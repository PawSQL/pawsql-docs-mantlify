...
Proceed in the following order (the first four steps are onboarding and data preparation; the rest are governance actions):
...
<Steps>
  ...
  <Step title="View patrol reports">
    Review risk distribution and top issues from the database-object perspective.
  </Step>
  <Step title="Track governance progress">
    Track each SQL via optimization status, disposition stage, and performance improvement.
  </Step>
</Steps>

```mermaid
flowchart LR
    A["Connect instances"] --> B["Collect slow SQL"]
    A --> C["Patrol database objects"]
    B --> D["Aggregate & deduplicate"]
    D --> E["Batch optimize"]
    C --> F["View patrol reports"]
    E --> G["Track progress"]
```
...
<CardGroup cols={2}>
  ...
  <Card title="Collect Slow SQL" href="/en/user-guide/performance/collect-slow-sql" />
