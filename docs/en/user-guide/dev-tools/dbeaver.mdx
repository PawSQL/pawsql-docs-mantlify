...
Install PawSQL for DBeaver from a software repository; see [Install IDE Extensions](/en/user-guide/installation/ide-plugins) for steps. After installation, confirm that **PawSQL Client** settings appear under Preferences.
...
<Frame caption="PawSQL Client settings in Preferences">
  <img src="\images\tools\dbeaver-plugin-preference-zh.png" />
</Frame>
...
<Frame caption="Risk prompt from the pre-execution review">
  <img src="\images\tools\dbeaver-plugin-review-en.png" alt="Risk prompt from the pre-execution review" />
</Frame>
...
With review enabled, the extension returns rule findings with severity and, when the configured threshold is reached, shows a risk prompt where you can revise the SQL, cancel, or proceed.

## Verification

Confirm the integration by running a read-only statement on a non-production datasource and checking that expected findings or the risk prompt appear, and that risky SQL is intercepted according to the configured threshold.
