...
through a shared intelligence layer: **SQL Review · SQL Rewrite · Index Advisor · Execution Plan Analysis · Performance Validation** — helping organizations build scalable SQL Engineering and SQL Governance capabilities.
...
Application code already has unit tests, code review, security scanning, and CI/CD. Database SQL should have the same level of engineering control. PawSQL can integrate SQL review and performance analysis into CI/CD:

```mermaid
flowchart LR
    A[Developer Commit] --> B[Pull Request] --> C[CI/CD]
    C --> D[SQL Review] --> E[Policy Evaluation] --> F{PASS / BLOCK}
```
...
PawSQL connects slow-query collection, SQL normalization, pattern grouping, performance analysis, SQL rewrite, index advice, and plan validation into one flow:
...
[Explore Database Migration SQL Governance →](/en/use-cases/index#sql-governance-for-database-migration)
...
| Migration Team | How do we reduce SQL performance regression during migration? | [Database Migration Governance](/en/use-cases/index#sql-governance-for-database-migration) |
...
<CardGroup cols={2}>
  ...
  <Card title="Database Migration Governance" icon="arrow-right-left" href="/en/use-cases/index#sql-governance-for-database-migration">
