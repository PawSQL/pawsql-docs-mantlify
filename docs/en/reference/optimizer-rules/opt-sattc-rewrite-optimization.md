---
id: en-optimizer-rule-opt-sattc-rewrite-optimization
title: SATTC Rewrite Optimization
type: reference
status: draft
owners: []
entityRef:
  type: rule
  id: opt-sattc-rewrite-optimization
tags:
- optimizer-rule
- rewrite
localeOf: optimizer-rule-opt-sattc-rewrite-optimization
subtype: rule
language: en
translationKey: optimizer-rule-opt-sattc-rewrite-optimization
layout: detail
product: pawsql
---

> **Generated file.** Do not edit by hand — change the source metadata (`metadata/rules/optimizer/*.yaml`) and re-run the generator.

| Field | Value |
|---|---|
| Rule ID | opt-sattc-rewrite-optimization |
| Name | SATTC Rewrite Optimization |
| Category | rewrite — Rewrite |
| Severity | info |
| Databases | Unverified (database scope not declared) |
