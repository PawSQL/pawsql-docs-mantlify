...
工作空间是 PawSQL 组织 SQL 分析上下文的基本单位。它记录目标数据库类型、版本、Schema 以及可用的表、列、约束和索引等元数据，供 SQL 审核、查询重写优化、智能索引推荐、执行计划分析和自动化性能验证使用。
...
了解 PawSQL 工作空间，并通过 DDL 或数据库连接为 SQL 审核和优化提供数据库上下文。

## 准备工作

创建工作空间需要提供 DDL 定义或数据库连接，以获取数据库类型、版本、Schema 及对象元数据。
...
如果只是快速分析一条 SQL，可以先通过 DDL 创建工作空间；如果需要长期审核、批量治理或自动化性能验证，优先考虑数据库连接方式。具体步骤见[通过 DDL 创建](/user-guide/workspaces/create-from-ddl)和[通过数据库连接创建](/user-guide/workspaces/create-from-database)。
...
一个配置了正确数据库类型、版本、Schema 和元数据的工作空间，供 SQL 审核、重写、索引和计划分析使用。

## 验证

确认工作空间的数据库类型、版本、Schema 和元数据与 SQL 真实运行环境一致；创建步骤见通过 DDL 创建和通过数据库连接创建的文档。
