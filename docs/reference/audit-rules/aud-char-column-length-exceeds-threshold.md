---
id: audit-rule-aud-char-column-length-exceeds-threshold
title: CHAR 字段长度超过阈值
type: reference
status: draft
owners: []
...
localeOf: en-audit-rule-aud-char-column-length-exceeds-threshold
subtype: rule
language: zh
...
| 字段 | 值 |
|---|---|
| 规则 ID | aud-char-column-length-exceeds-threshold |
| 规则名称 | CHAR 字段长度超过阈值 |
| 类别 | ddl（对象与结构定义） |
| 预警级别 | 提示 |
| 适用数据库 | 待验证（尚未声明数据库范围） |
