---
id: optimizer-rule-opt-npe-rewrite-null-pointer-exception-prevention
...
localeOf: en-optimizer-rule-opt-npe-rewrite-null-pointer-exception-prevention
subtype: rule
language: zh
...
| 字段 | 值 |
|---|---|
| 规则 ID | opt-npe-rewrite-null-pointer-exception-prevention |
| 规则名称 | NPE 重写（空指针异常预防） |
| 类别 | rewrite（重写） |
| 预警级别 | 警告 |
| 适用数据库 | 待验证（尚未声明数据库范围） |
