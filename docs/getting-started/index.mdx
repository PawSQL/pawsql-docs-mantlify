...
PawSQL 是一个面向开发人员、测试人员、数据库管理员与企业治理团队的 **SQL 工程与治理平台**，把规范审核、重写优化、索引推荐、性能验证与执行计划分析整合到统一平台，覆盖 SQL 从开发到生产的全生命周期。
...
<CardGroup cols={2}>
  <Card title="SQL 规范审核" icon="list-check" href="/reference/audit-rules/index">
    在 SQL 上线前，按开发规范与数据库最佳实践自动识别风险。
  </Card>
  <Card title="查询重写优化" icon="code-compare" href="/reference/optimizer-rules/index">
    基于语义等价约束调整查询结构，为优化器提供更优的执行方案。
  </Card>
  <Card title="智能索引推荐" icon="layer-group" href="/features/index-recommendation">
    结合过滤、关联、排序与统计信息，设计候选索引方案。
  </Card>
  <Card title="性能验证" icon="chart-line" href="/features/performance-validation">
    借助数据库优化器比较优化前后的代价、访问路径与执行计划。
  </Card>
  <Card title="执行计划可视化" icon="sitemap" href="/features/execution-plan-visualization">
...
对于更复杂的查询，PawSQL 还会分析子查询、关联、聚合与谓词下推等优化机会，在保持语义一致的前提下生成候选方案。

### 各项能力的关键检查项

<AccordionGroup>
  <Accordion title="SQL 规范审核">
    - SQL 编写规范
    - DDL 与 DML 风险
    - 查询性能风险
    - 索引使用问题
...
<Steps>
  ...
  <Step title="创建工作空间">
    指定数据库类型与版本，通过 DDL 或数据库连接，为 SQL 分析提供所需的数据库上下文。参见[工作空间与数据库上下文](/user-guide/workspaces/index)。
  </Step>
  <Step title="完成第一次优化">
...
<CardGroup cols={2}>
  ...
  <Card title="审核规则目录" icon="list-check" href="/reference/audit-rules/index">
    查询某条审核规范的依据。
  </Card>
  <Card title="优化算法目录" icon="code-compare" href="/reference/optimizer-rules/index">
    了解查询改写的判定依据。
  </Card>
