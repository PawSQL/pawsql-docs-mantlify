...
PawSQL SQL质量检查用于在 SQL 生命周期的早期阶段发现问题，包括语法与兼容性问题、企业规范违规、高风险 DDL/DML，以及可能导致性能退化的 SQL 写法。
...
传统 SQL质量检查通常依赖人工 DBA 评审、正则表达式或简单规则匹配。这类方式很难准确理解 SQL 的结构、对象关系和数据库语义，也难以覆盖复杂 SQL、存储过程以及多数据库环境。
...
因此，SQL质量检查的目标不是“找错误”，而是把 SQL 质量控制前移到研发和交付流程中。
...
| Capability | Simple SQL Lint | PawSQL SQL质量检查 |
|---|---|---|
| Syntax check | ✓ | ✓ |
