...
title: 查询重写优化
description: 使用 PawSQL 自动识别低效 SQL 写法，并生成语义等价且更高效的查询重写优化方案。
type: explanation
icon: git-compare
product: pawsql
status: published
...
PawSQL 查询重写优化通过 SQL 解析、语义分析与优化规则，将低效 SQL 自动转换为语义等价、但更有利于数据库优化器生成高效执行计划的形式。

与仅输出文字建议不同，PawSQL 的目标是直接生成可执行的优化 SQL，并进一步通过自动化性能验证判断重写是否有效。
