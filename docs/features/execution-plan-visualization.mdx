...
PawSQL Execution Plan Analysis 将数据库原生执行计划转换为更直观的图形结构，帮助开发者和 DBA 更快理解 SQL 的执行路径。

它不仅展示“有哪些算子”，还用于辅助识别 SQL 性能瓶颈、解释优化结果，并比较优化前后的执行计划差异。

<Note>
本页关注**计划的呈现与瓶颈定位**。优化前后的**代价验证与判定**见 [Performance Validation](/features/performance-validation)。
</Note>
